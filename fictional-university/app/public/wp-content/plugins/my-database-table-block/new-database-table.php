...
/*
  Plugin Name: Pet Adoption my Block (New DB Table)
  Version: 1.0
  Author: Brad
...
class PetAdoptionTablePlugin
{
  function __construct()
  {
    ...
    // add_filter('template_include', array($this, 'loadTemplate'), 99); //loading custom page template
  }
  ...
  function onActivate()
  {
    ...
    dbDelta("CREATE TABLE $this->tablename (
      ...
      petweight smallint NOT NULL DEFAULT 0,
...
class OurPluginPlaceholderBlock
{
  ...
  {
    ob_start();
    require plugin_dir_path(__FILE__) . 'petlist-block/' . $this->name . '.php';
    return ob_get_clean();
  }

  function onInit()
  {
    wp_register_script($this->name, plugin_dir_url(__FILE__) . "/petlist-block/{$this->name}.js", array('wp-blocks', 'wp-editor'));
