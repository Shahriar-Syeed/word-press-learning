...
/*
  Plugin Name: Pet Adoption BLOCK (New DB Table)
  Version: 1.0
  Author: Brad
...
class OurPluginPlaceholderBlock
{
  ...
  {
    ob_start();
    require plugin_dir_path(__FILE__) . 'pets-blocks/' . $this->name . '.php';
    return ob_get_clean();
  }

  function onInit()
  {
    wp_register_script($this->name, plugin_dir_url(__FILE__) . "/pets-blocks/{$this->name}.js", array('wp-blocks', 'wp-editor'));
